...
`;
      ...
      const result = filterFalsePositives(diagnostics, new Map([['test.svelte.tsx', content]]));
...
`;
      const diagnostics: Diagnostic[] = [
        makeDiag(2769, 'No overload matches this call', 'test.svelte.tsx', 3, 14),
      ];
...
`;
  ...
  describe('TS17001 duplicated JSX attributes', () => {
    test('should filter DUPLICATED_JSX_ATTRIBUTES', () => {
      ...
      const diagnostics: Diagnostic[] = [
        makeDiag(17001, "JSX elements cannot have multiple attributes with the same name.", 'test.svelte.tsx', 1, 21),
      ];
  ...
  describe('TS2300/TS1117 duplicate identifiers in JSX', () => {
    test('should filter DUPLICATE_IDENTIFIER in JSX attributes', () => {
      ...
      const diagnostics: Diagnostic[] = [
        makeDiag(2300, "Duplicate identifier 'value'.", 'test.svelte.tsx', 1, 18),
      ];
...
`;
      const diagnostics: Diagnostic[] = [
        makeDiag(2300, "Duplicate identifier 'value'.", 'test.svelte.tsx', 3, 7),
      ];
  ...
  describe('edge cases', () => {
    ...
    test('should handle empty content', () => {
      ...
      const result = filterFalsePositives(diagnostics, new Map([['test.svelte.tsx', '']]));
